function UnstructuredLandscape(
  ...
  var borderNum = 200;
  for (var k=0;k<borderNum;k++){
    var theta = 2*Math.PI/borderNum*k;
    var x = xInit + Math.cos(theta)*rInit*1.05;
    var y = yInit + Math.sin(theta)*rInit*1.05;
    set(x,y,minHeight);
  }

  for (var k=0;k<currentN;k++){
    var z = ndx.get(k,2);
    ndx.set(k,2,z-minHeight);
  }
  ...
  return {
    ...
    minMaxHeightWidth: [minHeight, maxHeight, rInit]
  };
}
